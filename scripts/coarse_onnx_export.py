...
def test_pytorch_model(model, codebook_tables):
    ...
    codes = torch.randint(
        0, model.vocab_size, (batch_size, model.n_codebooks, time_steps)
    )
...
def export_coarse_vampnet_simple(model_path, output_path):
    ...
    codebook_tables_path = (
        Path(__file__).parent.parent / "models_onnx/lac_codebook_tables.pth"
    )
    ...
    class EmbeddingLookupONNX(nn.Module):
        ...
        def forward(self, codes):
            ...
            for i in range(self.n_codebooks):
                ...
                if self.has_special:
                    ...
                    for name in self.special_names:
                        ...
                        special_embeds.append(
                            special_param[i : i + 1]
                        )  # (1, latent_dim)

                    special_embeds = torch.cat(
                        special_embeds, dim=0
                    )  # (n_special, latent_dim)
            ...
            latents = torch.cat(
                latents, dim=1
            )  # (batch, n_codebooks * latent_dim, time)
            return latents
