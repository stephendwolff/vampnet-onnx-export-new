...
class ControlEncoder(nn.Module):
    ...
    def __init__(
        self, ctrl_dims: dict[str, int], embedding_dim: int, cfg_dropout_prob: float
    ):
        super().__init__()
        self.ctrl_encoders = nn.ModuleDict(
            {key: nn.Linear(dim, embedding_dim) for key, dim in ctrl_dims.items()}
        )
    ...
    def forward(
        ...
        assert list(sorted(ctrls.keys())) == list(
            sorted(self.ctrl_encoders.keys())
        ), "ctrls and ctrl_encoders keys do not match"
        assert list(sorted(ctrl_masks.keys())) == list(
            sorted(self.ctrl_encoders.keys())
        ), "ctrl_masks and ctrl_encoders keys do not match"
        ...
        for ck in ctrls:
            ...
            assert (
                ctrld.shape[-1] == embedding.shape[-1]
            ), "ctrls should match x along time dimension"
            assert ctrlmask.ndim == 2, "ctrlmask should be 2d"
            assert (
                ctrlmask.shape[-1] == ctrld.shape[-1]
            ), "ctrlmask should match ctrld along time dimension"
...
class VampNet(at.ml.BaseModel):
    ...
    def generate(
        ...
        tocfgblank = lambda x: (
            torch.cat([x, torch.zeros_like(x)], dim=0) if use_cfg else x
        )
        ...
        if debug:
            ...
            plt.rcParams["figure.figsize"] = (
                20,
                0.4,
            )  # Default size for a 4x2000 grid (2000/100, 4/100)
            plt.rcParams["image.interpolation"] = (
                "nearest"  # Ensures no smoothing for imshow
            )
            plt.rcParams["image.aspect"] = "auto"  # Maintains proper aspect ratio
            plt.rcParams["axes.axisbelow"] = (
                True  # Ensures axis is beneath the data (for clarity)
            )
        ...
        for i in range(sampling_steps):
            ...
            if cfg_guidance is not None:
                ...
                logits_cond = cfg_guidance * logits_cond + cfg_guidance * (
                    1 - logits_uncond
                )
