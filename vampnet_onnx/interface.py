...
from .mask import *
from .vampnet_onnx import VampNetCoarseONNX, VampNetC2FONNX
...
class Interface(torch.nn.Module):
    ...
    def __init__(
        ...
        use_onnx_vampnet: bool = False,
        vampnet_coarse_embeddings_onnx_path: str = None,
        vampnet_coarse_transformer_onnx_path: str = None,
        vampnet_c2f_embeddings_onnx_path: str = None,
        vampnet_c2f_transformer_onnx_path: str = None,
    ):
        super().__init__()
        ...
        assert (
            lac_decoder_onnx_path is not None
        ), "must provide a decoder onnx checkpoint"
        assert (
            lac_quantizer_onnx_path is not None
        ), "must provide a quantizer onnx checkpoint"
        ...
        elif coarse2fine_ckpt is not None:
        ...
        if compile and not use_onnx_vampnet:
    ...
    def s2t(self, seconds: float):
        ...
        if isinstance(seconds, np.ndarray):
            return np.ceil(
                seconds * self.onnx_codec.sample_rate / self.onnx_codec.hop_length
            )
        else:
            return math.ceil(
                seconds * self.onnx_codec.sample_rate / self.onnx_codec.hop_length
    ...
    def t2s(self, tokens: int):
        """tokens to seconds"""
        return tokens * self.onnx_codec.hop_length / self.onnx_codec.sample_rate

    def to(self, device):
        self.device = device
        self.coarse.to(device)
        self.onnx_codec.to(device)
    ...
    def coarse_to_fine(
        ...
        **kwargs,
        ...
        if length % chunk_len != 0:
            ...
            mask = (
                torch.nn.functional.pad(mask, (0, pad_len), value=1)
                if mask is not None
                else None
            )
        ...
        if return_mask:
            return (
                fine_z[:, :, :length].clone(),
                apply_mask(fine_z, mask, self.c2f.mask_token)[0][:, :, :length].clone(),
            )
    ...
    def coarse_vamp(self, z, mask, return_mask=False, gen_fn=None, **kwargs):
        ...
        for chunk, mask_chunk in zip(cz_chunks, mask_chunks):
            cz_masked_chunk, mask_chunk = apply_mask(
                chunk, mask_chunk, self.coarse.mask_token
            )
    ...
    def build_mask(
        ...
        pmask = periodic_mask(
            z, periodic_prompt, periodic_prompt_width, random_roll=True
        )
        mask = mask_and(mask, pmask)

        if onset_mask_width > 0:
            assert sig is not None, f"must provide a signal to use onset mask"
            mask = mask_and(mask, onset_mask(sig, z, self, width=onset_mask_width))
    ...
    def vamp(
        ...
        **kwargs,
        ...
        for i in range(feedback_steps):
            zv, mask_z = self.coarse_vamp(zv, mask=mask, return_mask=True, **kwargs)
            # roll the mask around a random amount
            mask_z = mask_z.roll(shifts=(i + 1) % feedback_steps, dims=-1)

        # add the top codebooks back in
        if zv.shape[1] < z.shape[1]:
            logging.debug(f"adding {z.shape[1] - zv.shape[1]} codebooks back in")
            zv = torch.cat([zv, z[:, self.coarse.n_codebooks :, :]], dim=1)

        # now, coarse2fine
        logging.debug(f"coarse2fine!")
        zv, fine_zv_mask = self.coarse_to_fine(
            zv, mask=mask, typical_filtering=True, _sampling_steps=2, return_mask=True
        )
        mask_z = torch.cat(
            [
                mask_z[:, : self.coarse.n_codebooks, :],
                fine_zv_mask[:, self.coarse.n_codebooks :, :],
            ],
            dim=1,
        )
...
if __name__ == "__main__":
    ...
    zv, mask_z = interface.coarse_vamp(
        z, mask=mask, return_mask=True, gen_fn=interface.coarse.generate
    )
